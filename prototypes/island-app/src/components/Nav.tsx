...
const TABS: { key: Screen; label: string; icon: JSX.Element }[] = [
  ...
  {
    /* "Decorate", not "Shop": it's where the island gets built, and the
       currency is focus, not money. The route key stays "shop". */
    key: "shop", label: "Decorate",
...
export function Nav({ screen, onGo, hidden }: { screen: Screen; onGo: (s: Screen) => void; hidden: boolean }) {
  return (
    <nav id="nav" className={hidden ? "hidden" : ""} aria-label="Main">
      {/* one pill that glides between tabs, rather than one that blinks */}
      <span id="nav-pill" aria-hidden="true"
        style={{ ["--i" as never]: Math.max(0, TABS.findIndex(t => t.key === screen)) as never }} />
      {TABS.map(t => (
        <button key={t.key} data-nav={t.key} data-sfx="own" className={screen === t.key ? "on" : ""}
